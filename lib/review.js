...
export function formatReviewMarkdown(report) {
  const result = report.approved
    ? 'No critical or high-risk changes detected'
    : 'Review required before applying this configuration';
  const lines = [
    '## MCP Configuration Change Review',
    '',
    `**Result:** ${result}.`,
    '',
    '| Severity | Count |',
  ...
  if (report.changes.length > 0) {
    lines.push('', '### Server changes', '', '| Server | Change |', '| --- | --- |');
    for (const change of report.changes) {
      lines.push(`| ${escapeMarkdownCell(change.server)} | ${titleCase(change.kind)} |`);
    }
  }

  if (report.findings.length > 0) {
    lines.push('', '### Findings', '', '| Severity | Server | Finding | Recommended review |', '| --- | --- | --- | --- |');
    for (const item of report.findings) {
      lines.push(`| ${titleCase(item.severity)} | ${escapeMarkdownCell(item.server)} | ${escapeMarkdownCell(item.message)} | ${escapeMarkdownCell(item.recommendation)} |`);
    }
  } else {
    lines.push('', 'No configuration differences were detected.');
  }

  lines.push('', '> Secret values are intentionally redacted. This report is a change-review aid, not a guarantee of safety.');
  return lines.join('\n');
}
